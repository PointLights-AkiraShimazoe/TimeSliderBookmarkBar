...
class BookmarkLabelOverlay(QtWidgets.QWidget):
    ...
    def event(self, e):
        if e.type() == QtCore.QEvent.ToolTip:
            ...
            return True
        # NOTE: importlib.reload 後に旧クラスの残存インスタンスが event を受けても
        #       壊れないよう、モジュールグローバル名 (BookmarkLabelOverlay) を参照する
        #       super(Class, self) ではなく、基底クラスを直接呼ぶ（reload 安全）。
    ...
    def leaveEvent(self, event):
        ...
        self.update()
        # reload 安全のため基底クラスを直接呼ぶ（super(Class, self) を使わない）
...
def _sweep_overlays(keep=None):
    """OBJECT_NAME を持つオーバーレイをアプリ全体から掃除する。

    reload 等で親から外れた「迷子」のオーバーレイ（旧クラスの残存インスタンス）を確実に破棄する。
    keep に指定したウィジェットだけは残す（現行 _INSTANCE を保持したい場合に使用）。
    """
    try:
    ...
    for w in widgets:
        try:
            if w is not None and w is not keep and w.objectName() == OBJECT_NAME:
                try:
                    w.hide()
                except Exception:
                    pass
...
/* 縦スライダーは ● より下側を青(塗り)にする（add-page=下側 / sub-page=上側） */
QSlider::add-page:vertical { background: #4aa3c7; border-radius: 3px; }
QSlider::sub-page:vertical { background: #232323; border-radius: 3px; }
...
def _recover_panel():
    """reload 後などで _PANEL 参照が失われた場合に、既存のパネルウィジェットを拾い直す。"""
    # 注意: reload 後は旧クラスのインスタンスなので isinstance(BookmarkControlPanel) は使えない。
    #       objectName と barBtn 属性の有無（ダックタイピング）で判定する。
    try:
        for w in list(QtWidgets.QApplication.allWidgets()):
            try:
                if w is not None and w.objectName() == PANEL_OBJECT_NAME \
                        and hasattr(w, "barBtn"):
                    return w
            except Exception:
                pass
    except Exception:
...
def show_controls(bottom_ratio=DEFAULT_BOTTOM_RATIO, inset=None,
    ...
    if already:
        ...
        try:
            ...
            pass
        # reload 後は _PANEL 参照が失われるため、既存パネルを拾い直す
        if _PANEL is None:
            _PANEL = _recover_panel()
        # バーが ON だった場合はオーバーレイを作り直す（reload で消えた分を復帰）
...
# -----------------------------------------------------------------------------
# モジュール読み込み（importlib.reload 含む）時に、前バージョンの残存オーバーレイを
# 即座に掃除する。これにより、シェルフからのリロードだけで「旧クラスの古い event が
# 走り続けて super() エラーを出す」問題が解消する（Maya 再起動を待たなくてよい）。
# reload では本モジュールのトップレベルが再実行され _INSTANCE は None に戻るため、
# ここで全オーバーレイを破棄しても現行の生きたインスタンスを誤って消すことはない。
# -----------------------------------------------------------------------------
try:
    _sweep_overlays()
except Exception:
